...
// 2. Process
		...
		// 2.1 Generate the background to substract
		run("Z Project...", "projection=Median");
		bkg = getImageID();
		rename("bkg");
		// 2.2 Calculate the difference
		imageCalculator("Difference create stack", "median","bkg");
		rename("mascara");
		...
		close("bkg");
		close("median");
		//2.4 Loop for every temporal frame
			for (t = 0; t < frames; t++) {
 
				selectImage(mascara);
				Stack.setFrame(t+1);
				run("Duplicate...", "title=temp_frame");
		
		//2.5 Filters particles
				run("Analyze Particles...", "size=20000-Infinity show=Masks display add");
				run("Grays");
				...
				// 2.6 Measure several features in the frame
...
// 2.8 Write the results of the frame in the table			
